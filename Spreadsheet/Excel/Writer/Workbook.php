...
class Spreadsheet_Excel_Writer_Workbook extends Spreadsheet_Excel_Writer_BIFFwriter
{
    ...
    protected function _storeOLEFile()
    {
        ...
        // close and delete all worksheet files
        for ($i = 0; $i < $total_worksheets; $i++) {
            $worksheet = $this->_worksheets[$i];
            if ($worksheet->_tmp_file != '') {
                if ($worksheet->_filehandle) {
                    fclose($worksheet->_filehandle);
                    $worksheet->_filehandle = '';
                }
                @unlink($worksheet->_tmp_file);
                $worksheet->_tmp_file      = '';
                $worksheet->_using_tmpfile = true;
            }
        }
